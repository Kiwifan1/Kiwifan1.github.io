...
describe('calculateThermoelectricBoilerCost', () => {
  it('produces the expected metrics for a max-sized boiler', () => {
    ...
    expect(result.superheatingElements).toBe(671);
    const waterCapacity = result.waterVolume * 16_000;
    const boilCapacity = result.superheatingElements * 100_000;
    const steamCapacity = result.steamVolume * 160_000;
    expect(waterCapacity).toBe(67_024_000);
    expect(boilCapacity).toBe(67_100_000);
  ...
  it('clamps inputs and respects valve replacements', () => {
    ...
    expect(result.superheatingElements).toBe(3);
    expect(result.waterVolume).toBe(15);
