...
export const TURBINE_LEGEND: LegendEntry[] = [
...
export const BOILER_LEGEND: LegendEntry[] = [
...
interface LayerDraft extends Omit<LayerVisual, 'materials'> {}

export function buildTurbineLayers(plan: TurbineSupportPlan): LayerVisual[] {
  ...
  for (let i = 0; i < rotorLayers; i += 1) {
    layers.push({
      ...
      description:
        'Stack the rotor shaft and attach the paired rotor blades opposite each other (repeat for each rotor layer).',
...
export function buildBoilerLayers(plan: BoilerSupportPlan): LayerVisual[] {
  const layers: LayerDraft[] = [];
  const geometry = plan.construction.perUnit.geometry;
...
export function summarizeMaterials(grid: CellVisual[][]): LayerMaterial[] {
...
function summarizeRectangularInterior(
  width: number,
  length: number,
  interior: CellType,
  edge: CellType = 'casing',
...
function createFilledInterior(
  ...
  return summarizeRectangularInterior(size, size, type, edge, corner);
}
...
function createRectGrid(
  ...
  return summarizeRectangularInterior(width, length, interior, edge, corner);
}
...
function createRectFilledInterior(
  ...
  return summarizeRectangularInterior(width, length, type, edge, corner);
}
